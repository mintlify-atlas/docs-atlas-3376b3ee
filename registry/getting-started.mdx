...
If you're using a different build system, you can skip this step as long as your build system produces valid JSON files that conform to the [registry-item schema specification](/registry/registry-item-json).
...
This `registry.json` file must conform to the [registry schema specification](/registry/registry-json).
...
```svelte title="registry/hello-world/hello-world.svelte"
<script lang="ts">
...
You can read more about the registry item schema and file types in the [registry item schema docs](/registry/registry-item-json).
